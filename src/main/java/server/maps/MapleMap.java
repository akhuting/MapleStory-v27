...
//        }
    ...
    public boolean cangetNXMap() {
        ...
        return this.getId() >= 952000000 && this.getId() <= 954061000 || this.getId() >= 925020100 && this.getId() <= 925033000;
...
//            FileoutputUtil.log("新控制 " + monster.getObjectId());
    ...
    public int getRunesSize() {
        return this.mapobjects.get(MapleMapObjectType.MAPLE_RUNE).size();
    }
...
//        }
    ...
    public List<MapleArrowsTurret> getArrowsTurretsThreadsafe() {
        ArrayList ret = new ArrayList();
        this.mapobjectlocks.get(MapleMapObjectType.ARROWS_TURRET).readLock().lock();
        try {
            for (MapleMapObject mmo : mapobjects.get(MapleMapObjectType.ARROWS_TURRET).values()) {
                ret.add(mmo);
            }
        } finally {
            this.mapobjectlocks.get(MapleMapObjectType.ARROWS_TURRET).readLock().unlock();
        }
        return ret;
    }
