...
public class MapleTrade {
    ...
    public void CompleteTrade() {
        ...
        if (this.exchangeItems != null) {
            ...
            for (Item item : itemz) {
                ...
                FileoutputUtil.log("[交易] " + this.chr.get().getName() + " 交易获得道具: " + item.getItemId() + " x " + item.getQuantity() + " - " + ii.getName(item.getItemId()));
            }
        ...
        if (this.exchangeMeso > 0) {
            ...
            FileoutputUtil.log("[交易] " + this.chr.get().getName() + " 交易获得金币: " + this.exchangeMeso);
        }
        this.exchangeMeso = 0;
        this.chr.get().getClient().getSession().write(TradePacket.TradeMessage(this.tradingslot, (byte) 8));
    }
    ...
    public void setMeso(int meso) {
        ...
        if (this.chr.get().getMeso() >= meso) {
            ...
            this.meso += meso;
            this.chr.get().getClient().getSession().write(TradePacket.getTradeMesoSet((byte) 0, this.meso));
    ...
    public void addItem(Item item) {
        ...
        this.items.add(item);
        this.chr.get().getClient().getSession().write(TradePacket.getTradeItemAdd((byte) 0, item));
        if (this.partner != null) {
    ...
    public void chat(String message) {
        if (!CommandProcessor.processCommand(this.chr.get().getClient(), message, CommandType.TRADE)) {
        ...
        if (this.chr.get().getClient().isMonitored()) {
            WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverMessageRedText(this.chr.get().getName() + " 在交易中对 " + this.partner.getChr().getName() + " 说: " + message));
    ...
    public void chatAuto(String message) {
        ...
        if (this.chr.get().getClient().isMonitored()) {
            WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverMessageRedText( this.chr.get().getName() + " said in trade [Automated] with " + this.partner.getChr().getName() + " 说: " + message));
        } else if ((this.partner != null) && (this.partner.getChr() != null) && (this.partner.getChr().getClient().isMonitored())) {
            WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverMessageRedText(this.chr.get().getName() + " said in trade [Automated] with " + this.partner.getChr().getName() + " 说: " + message));
        }
    }
