...
public class WrodlPartyService {
    ...
    public MapleParty getParty(int partyId) {
        return this.partyList.get(partyId);
    }

    public MapleExpedition getExped(int partyId) {
        return this.expedsList.get(partyId);
    }
    ...
    public MapleExpedition disbandExped(int partyId) {
        ...
        MapleExpedition ret = this.expedsList.remove(Integer.valueOf(partyId));
        Iterator i$;
        if (ret != null) {
    ...
    public MapleParty disbandParty(int partyId) {
        ...
        MapleParty ret = this.partyList.remove(Integer.valueOf(partyId));
        if (ret == null) {
            return null;
        }
        ret.disband();
    ...
    public List<PartySearch> searchParty(PartySearchType pst) {
        return this.searcheList.get(pst);
    }

    public void removeSearch(PartySearch ps, String text) {
        List ss = this.searcheList.get(ps.getType());
        if (ss.contains(ps)) {
            ss.remove(ps);
            ps.cancelRemoval();
            if (ps.getType().exped) {
    ...
    public void addSearch(PartySearch ps) {
        this.searcheList.get(ps.getType()).add(ps);
    }
