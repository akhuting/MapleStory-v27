        ...
        switch (action) {
            case 创建:
                ...
                if (createType == 4) {
                    ...
                    if ((!chr.getMap().getMapObjectsInRange(chr.getTruePosition(), 20000.0D, Arrays.asList(MapleMapObjectType.SHOP, MapleMapObjectType.HIRED_MERCHANT)).isEmpty()) || (!chr.getMap().getPortalsInRange(chr.getTruePosition(), 20000.0D).isEmpty())) {
...
//                        ips.setAvailable(true);
            ...
            case 雇佣商店_购买道具0026:
                ...
                MaplePlayerShopItem tobuy = shop.getItems().get(item1);
                if (tobuy == null) {
            ...
            case 移除物品:
                ...
                MaplePlayerShopItem item2 = shop1.getItems().get(slot1);
                if ((item2 != null)
                        && (item2.bundles > 0)) {
