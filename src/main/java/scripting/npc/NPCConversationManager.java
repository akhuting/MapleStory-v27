            ...
            for (i$ = skillIds.iterator(); i$.hasNext();) {
                ...
                for (Entry<Skill, SkillEntry> data : sDate.entrySet()) {
                    if (data.getKey().getId() == skillId) {
                        String txt = new StringBuilder().append("public static final int ").append(data.getKey().getName()).append(" = ").append(data.getKey().getId()).append("; //技能最大等级").append(data.getKey().getMaxLevel()).toString();
