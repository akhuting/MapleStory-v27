...
public class QuestScriptManager extends AbstractScriptManager {
    ...
    public void startQuest(MapleClient c, int npcId, int questId) {
        try {
            ...
            c.setClickedNPC();
            iv.invokeFunction("start", (byte) 1, (byte) 0, (int) (byte) 0);
    ...
    public void startAction(MapleClient c, byte mode, byte type, int selection) {
        QuestActionManager qm = this.qms.get(c);
        if (qm == null) {
            return;
        }
        try {
...
//                }
                ...
                c.getPlayer().setConversation(1);
                c.setClickedNPC();
                iv.invokeFunction("end", (byte) 1, (byte) 0, (int) (byte) 0);
            } else {
                dispose(c);
            }
        } catch (ScriptException | NoSuchMethodException e) {
    ...
    public void dispose(MapleClient c) {
        ...
        if (qm != null) {
            this.qms.remove(c);
            c.removeScriptEngine("脚本/任务/" + qm.getQuest() + ".js");
        }
