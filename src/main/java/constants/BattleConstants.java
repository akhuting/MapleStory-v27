...
public class BattleConstants {
            ...
            for (int i = 0; i < mob.evolutions.size(); i++) {
                ...
                if (!pokedex.containsKey(id)) {
                    ...
                    if (mtm != null) {
                        ...
                        for (Pair mt : mtm) {
                            pe.maps.add(new Pair(mt.left, (int) Math.round(1.0D / mapsToMobs.get(getMap(((Integer) mt.left))).size() / ((Integer) mt.right) * 10000.0D)));
                        }
                    } else {
                        pe.maps = null;
                    }
    ...
    public enum PokemonNature {
        ...
        PokemonNature(BattleConstants.PokemonStat inc, BattleConstants.PokemonStat dec) {
            this.inc = inc;
            this.dec = dec;
        }
    ...
    public enum PokemonMob {
        ...
        PokemonMob(BattleConstants.PokemonItem evoItem, BattleConstants.MobExp type, BattleConstants.PokemonAbility ability1, BattleConstants.PokemonAbility ability2, Integer[] evo) {
            this.type = type;
